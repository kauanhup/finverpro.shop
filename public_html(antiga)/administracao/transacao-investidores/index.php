...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM investidores WHERE id_usuario = :id");
  ...
  <div class="pc-container">
          ...
          <div class="card table-card">
            <div class="card-header">
              <div class="d-sm-flex align-items-center justify-content-between">
                <h5 class="mb-3 mb-sm-0">Tenha controle de todas as transações dos investidores da Apple</h5>
              </div>
            </div>
    ...
    <thead>
        <tr>
            ...
            <th>DATA/HORA</th>
            <th>TELEFONE</th>
            <th>PRODUTO INVESTIDO</th>
            <th>ULTIMO CICLO</th>
            <th>RENDA TOTAL</th>
            <th>CICLO RENDIMENTO</th>
            <th>AÇÃO</th>
        </tr>
    </thead>
    <tbody>
        ...
        $query = $conn->query("SELECT * FROM investidores");

        while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
            echo "<tr>
                    <td>{$row['id_usuario']}</td>
                    <td>{$row['data_investimento']}</td>
                    <td>{$row['numero_telefone']}</td>
                    <td>{$row['produto_investido']}</td>
                    <td>{$row['ultimo_ciclo']}</td>
                    <td>R$ {$row['renda_total']}</td>
                    <td>{$row['ciclo_rendimento']}</td>
                    <td>
                        <a href='javascript:void(0);' onclick='deleteRecord({$row['id_usuario']})' class='avtar avtar-xs btn-link-secondary'>
