...
$sql = "SELECT u.id, u.data_criacao, u.telefone, u.saldo, u.saldo_comissao,
        (SELECT COUNT(*) FROM usuarios WHERE referencia_convite = u.codigo_referencia) AS convidados,
        (SELECT SUM(n.nivel_1 + n.nivel_2 + n.nivel_3) FROM niveis_convite n WHERE n.user_id = u.id) AS depositantes,
        (SELECT COUNT(*) FROM investidores WHERE id_usuario = u.id) AS produtos,
        (SELECT SUM(n.total_nivel1 + n.total_nivel2 + n.total_nivel3) FROM niveis_convite n WHERE n.user_id = u.id) AS depositos,
        (SELECT SUM(valor) FROM saques_comissao WHERE user_id = u.id AND status = 'Aprovado') AS saques_comissao
        FROM usuarios u";
  ...
  <div class="loading-text">Carregando recursos, aguarde um pouco...</div>
   ...
   <thead>
      <tr>
         ...
         <th>CONVIDADOS</th>
         <th>DEPOSITANTES</th> <!-- Alterado para Depositantes -->
         <th>PRODUTOS</th>
         <th>DEPÓSITOS</th> <!-- Alterado para a nova consulta -->
         <th>SAQUES</th>
         <th>AÇÃO</th>
      </tr>
   </thead>
   <tbody>
      ...
      <tr>
         ...
         <td>R$ <?= number_format($usuario['depositos'], 2, ',', '.') ?></td> <!-- Soma dos totais de nível -->
