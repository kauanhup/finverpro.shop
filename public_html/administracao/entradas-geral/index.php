...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($_GET['action'] === 'approve') {
        // Obter o valor do pagamento e o user_id
        $stmt = $conn->prepare("SELECT user_id, valor FROM pagamentos WHERE id = :id AND status = 'Pendente'");
        $stmt->execute(['id' => $id]);
        ...
        if ($pagamento) {
            ...
            $stmt = $conn->prepare("UPDATE pagamentos SET status = 'Aprovado' WHERE id = :id");
            $stmt->execute(['id' => $id]);
...
$stmt = $conn->query("SELECT id, user_id, valor, cod_referencia, status, data FROM pagamentos");
  ...
  <div class="pc-container">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><?= htmlspecialchars(date('d-m-Y H:i', strtotime($pagamento['data']))) ?></td>
            <td><?= htmlspecialchars($pagamento['cod_referencia']) ?></td>
...
<script>
    ...
    Swal.fire({
        icon: 'success',
        title: 'Sucesso',
        text: '<?= htmlspecialchars($_GET['mensagem']) ?>',
    ...
    // Função para aprovação de pagamento
