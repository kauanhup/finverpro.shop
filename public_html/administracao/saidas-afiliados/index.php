...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($_GET['action'] === 'approve') {
        // Aprovar o saque
        $stmt = $conn->prepare("UPDATE saques_comissao SET status = 'Aprovado' WHERE id = :id");
        $stmt->execute(['id' => $id]);
        ...
        echo "<script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    title: 'Aprovado!',
                    text: 'O saque foi aprovado com sucesso.',
                    icon: 'success',
        ...
        $stmt = $conn->prepare("SELECT user_id, valor FROM saques_comissao WHERE id = :id");
        $stmt->execute(['id' => $id]);
        ...
        if ($saque) {
            ...
            $stmt = $conn->prepare("UPDATE usuarios SET saldo_comissao = saldo_comissao + :valor WHERE id = :user_id");
            $stmt->execute(['valor' => $valor, 'user_id' => $user_id]);

            // Atualizar o status do saque para Cancelado
            $stmt = $conn->prepare("UPDATE saques_comissao SET status = 'Rejeitado' WHERE id = :id");
            $stmt->execute(['id' => $id]);
            ...
            echo "<script>
                document.addEventListener('DOMContentLoaded', function() {
                    Swal.fire({
                        title: 'Rejeitado!',
                        text: 'O saque foi cancelado e o saldo devolvido ao usuário.',
                        icon: 'success',
                        confirmButtonText: 'OK'
                    }).then((result) => {
                        if (result.isConfirmed) {
        ...
        $stmt = $conn->prepare("DELETE FROM saques_comissao WHERE id = :id");
        $stmt->execute(['id' => $id]);
        ...
        echo "<script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    title: 'Deletado!',
                    text: 'O registro de saque foi excluído com sucesso.',
                    icon: 'success',
                    confirmButtonText: 'OK'
                }).then((result) => {
                    if (result.isConfirmed) {
...
// Consultar saques
$stmt = $conn->query("SELECT id, user_id, tipo_pix, chave_pix, numero_telefone, nome_titular, valor, status, data FROM saques_comissao");
          ...
          <div class="row align-items-center">
            <div class="col-md-12">
              <ul class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">Saidas de Afiliados</li>
              </ul>
            </div>
            <div class="col-md-12">
              <div class="page-header-title">
                <h2 class="mb-0">Saidas de Pix</h2>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="card table-card">
            <div class="card-header">
              <div class="d-sm-flex align-items-center justify-content-between">
                <h5 class="mb-3 mb-sm-0">Tenha controle de todos as saidas de afiliados da plataforma.</h5>
              </div>
            </div>
               ...
               <table class="table table-hover" id="pc-dt-simple">
                  ...
                  <tbody>
                      ...
                      <tr>
                          ...
                          <td><?= htmlspecialchars($saque['tipo_pix']) ?></td>
                          <td><?= htmlspecialchars($saque['chave_pix']) ?></td>
                          <td>R$ <?= number_format($saque['valor'], 2, ',', '.') ?></td>
                          <td>
                              <span class="badge text-bg-<?= $saque['status'] === 'Aprovado' ? 'success' : ($saque['status'] === 'Rejeitado' ? 'danger' : 'warning') ?>">
                                  <?= strtoupper(htmlspecialchars($saque['status'])) ?>
                              </span>
                          </td>
                          <td>
    <?php if ($saque['status'] !== 'Aprovado'): ?>
        <a href="javascript:void(0);" class="avtar avtar-xs btn-link-secondary" onclick="approveSaque(<?= $saque['id'] ?>, this)">
            <i class="ti ti-checkbox f-20 bg-light-success"></i>
        </a>
    <?php else: ?>
        <a href="javascript:void(0);" class="avtar avtar-xs btn-link-secondary disabled">
            <i class="ti ti-checkbox f-20 bg-light-success" style="opacity: 0.5;"></i>
        </a>
    <?php endif; ?>

    <?php if ($saque['status'] !== 'Rejeitado'): ?>
        <a href="javascript:void(0);" class="avtar avtar-xs btn-link-secondary" onclick="rejectSaque(<?= $saque['id'] ?>)">
            <i class="ti ti-square-x f-20 bg-light-primary"></i>
        </a>
    <?php else: ?>
        <a href="javascript:void(0);" class="avtar avtar-xs btn-link-secondary disabled">
            <i class="ti ti-square-x f-20 bg-light-primary" style="opacity: 0.5;"></i>
        </a>
    <?php endif; ?>
    
    <a href="?action=delete&id=<?= $saque['id'] ?>" class="avtar avtar-xs btn-link-secondary">
        <i class="ti ti-trash f-20 bg-light-danger"></i>
              ...
              <script>
               function approveSaque(id, element) {
    Swal.fire({
        title: 'Você tem certeza?',
        text: "Esta ação irá aprovar o saque.",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            element.querySelector('i').style.opacity = '0.5';
            
            // Redireciona para aprovar o saque
...
function rejectSaque(id) {
    Swal.fire({
        title: 'Você tem certeza?',
        text: "Esta ação irá cancelar o saque e devolver o saldo.",
        icon: 'warning',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            // Redireciona para cancelar o saque
