...
// Buscar estatísticas básicas de pagamentos
try {
    $stats = [
        'pagamentos_hoje' => $db->fetchOne("SELECT COUNT(*) as total FROM pagamentos WHERE DATE(data_criacao) = CURDATE()")['total'] ?? 0,
        'valor_hoje' => $db->fetchOne("SELECT SUM(valor) as total FROM pagamentos WHERE DATE(data_criacao) = CURDATE() AND status = 'aprovado'")['total'] ?? 0,
        'pendentes' => $db->fetchOne("SELECT COUNT(*) as total FROM pagamentos WHERE status = 'pendente'")['total'] ?? 0,
    ];
} catch (Exception $e) {
    $stats = ['pagamentos_hoje' => 0, 'valor_hoje' => 0, 'pendentes' => 0];
}
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <title>Gestão de Pagamentos - Finver Pro</title>
    ...
    <div class="admin-layout">
        ...
        <main class="main-content">
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    <div class="stat-value"><?= $stats['pagamentos_hoje'] ?></div>
                    <div class="stat-label">Pagamentos Hoje</div>
                </div>
                
                <div class="stat-card">
                    <div class="stat-value">R$ <?= number_format($stats['valor_hoje'], 2, ',', '.') ?></div>
