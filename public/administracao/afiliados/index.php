...
// Buscar estatísticas básicas de afiliados
try {
    $stats = [
        'total_afiliados' => $db->fetchOne("SELECT COUNT(*) as total FROM usuarios WHERE EXISTS (SELECT 1 FROM usuarios u2 WHERE u2.indicado_por = usuarios.id)")['total'] ?? 0,
        'comissoes_pendentes' => $db->fetchOne("SELECT COUNT(*) as total FROM comissoes WHERE status = 'pendente'")['total'] ?? 0,
        'valor_comissoes_mes' => $db->fetchOne("SELECT SUM(valor_comissao) as total FROM comissoes WHERE MONTH(data_comissao) = MONTH(CURRENT_DATE) AND YEAR(data_comissao) = YEAR(CURRENT_DATE)")['total'] ?? 0,
    ];
} catch (Exception $e) {
    $stats = ['total_afiliados' => 0, 'comissoes_pendentes' => 0, 'valor_comissoes_mes' => 0];
}
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <title>Gestão de Afiliados - Finver Pro</title>
    ...
    <div class="admin-layout">
        ...
        <main class="main-content">
            <div class="page-header">
                <h1 class="page-title">Gestão de Afiliados</h1>
            </div>
            
            <!-- Estatísticas básicas -->
            <div class="stats-grid">
                <div class="stat-card">
                    <div class="stat-value"><?= $stats['total_afiliados'] ?></div>
                    <div class="stat-label">Total de Afiliados</div>
                </div>
                
                <div class="stat-card">
                    <div class="stat-value"><?= $stats['comissoes_pendentes'] ?></div>
            ...
            <div class="coming-soon">
                <i class="fas fa-user-friends"></i>
                <h3>Módulo de Afiliados</h3>
                <p>Este módulo está sendo desenvolvido e em breve estará disponível com funcionalidades completas para gestão de afiliados, comissões e rede de indicações.</p>
            </div>
        </main>
    </div>
</body>
