...
// Consultar os usuários afiliados com a nova estrutura
$sql = "SELECT u.id, u.created_at as data_criacao, u.telefone, 
        ...
        COALESCE((SELECT COUNT(DISTINCT cm.origem_usuario_id) FROM comissoes cm WHERE cm.usuario_id = u.id), 0) AS depositantes,
        (SELECT COUNT(*) FROM investimentos WHERE usuario_id = u.id) AS produtos,
        COALESCE((SELECT SUM(cm.valor_comissao) FROM comissoes cm WHERE cm.usuario_id = u.id AND cm.status = 'pago'), 0) AS comissoes_pagas,
        COALESCE((SELECT SUM(valor_liquido) FROM operacoes_financeiras WHERE usuario_id = u.id AND tipo = 'saque' AND status = 'aprovado'), 0) AS saques_comissao
        FROM usuarios u
        LEFT JOIN carteiras c ON u.id = c.usuario_id
        WHERE (SELECT COUNT(*) FROM usuarios WHERE referenciado_por = u.id) > 0
                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item" aria-current="page">Afiliados</li>
                        </ul>
                    </div>
                    <div class="col-md-12">
                        <div class="page-header-title">
                            <h2 class="mb-0">Todos os afiliados</h2>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card table-card">
                    <div class="card-header">
                        <div class="d-sm-flex align-items-center justify-content-between">
                            <h5 class="mb-3 mb-sm-0">Tenha controle de todos os afiliados da plataforma.</h5>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    <?php foreach ($usuarios as $usuario): ?>
                                        <tr>
                                            ...
                                            <td>R$ <?= number_format($usuario['saldo_comissao'], 2, ',', '.') ?></td>
                                            <td><?= htmlspecialchars($usuario['convidados']) ?></td>
                                            <td><?= htmlspecialchars($usuario['depositantes']) ?></td>
                                            <td><?= htmlspecialchars($usuario['produtos']) ?></td>
                                            <td>R$ <?= number_format($usuario['comissoes_pagas'], 2, ',', '.') ?></td>
                                            <td>R$ <?= number_format($usuario['saques_comissao'], 2, ',', '.') ?></td>
                                            <td>
